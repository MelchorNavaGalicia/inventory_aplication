...
{
    public class GetCategoriesByHandlerTests
    {
        private readonly Mock<ICategoryRepository> _categoryRepoMock;
        private readonly GetCategoriesByHandler _handler;

        public GetCategoriesByHandlerTests()
        {
            _categoryRepoMock = new Mock<ICategoryRepository>();
            _handler = new GetCategoriesByHandler(_categoryRepoMock.Object);
        }
        ...
        public async Task Handle_ShouldReturnPagedResultWithFilter()
        {
            var pagedResult = new PagedResult<CategoryResponseDto>(
                ...
                pageNumber: 1,
                pageSize: 10,
                items: new List<CategoryResponseDto> { new CategoryResponseDto { Id = 1, Name = "Filtered" } }
            );

            _categoryRepoMock.Setup(x => x.GetFilterPagedAsync(1, 10, "Filtered", It.IsAny<CancellationToken>()))
                .ReturnsAsync(pagedResult);

            var query = new GetCategoriesByQuery("Filtered", 1, 10);
